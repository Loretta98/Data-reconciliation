...
def knn_outlier_detection(input_path, window_sizes, values, merged_df_path,k):
    ...
    for filename in files:
        ...
        knn = NearestNeighbors(n_neighbors=window_sizes[k])  # Choose the number of neighbors
        knn.fit(data_scaled)
        ...
        plt.legend()
        # Save the plot
        plot_path = os.path.join(outliers_knn_path, f"plot_distance_threshold_{file_basename}.png")
        plt.savefig(plot_path)
        plt.close()
